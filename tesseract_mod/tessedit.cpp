...
bool Tesseract::init_tesseract_lang_data(
  ...
  if (oem==OEM_DEFAULT)
      {
      int i;
      static char *exts[] = {"size","params","nn","word-freq",""};
      for (i=0;exts[i][0]!='\0';i++)
          {
          STRING cubetest;
          FILE *f;
          cubetest = language_data_path_prefix + "cube." + exts[i];
          f=fopen(cubetest.string(),"r");
          gotcube= (f!=NULL);
          if (f!=NULL)
              fclose(f);
          if (gotcube)
