...
void SquishedDawg::read_squished_dawg(FILE *file,
  ...
  edges_ = (EDGE_ARRAY) memalloc(sizeof(EDGE_RECORD) * num_edges_);
    /* willus.com mod */
    /*
    ** Very weird issue running XP version in virtual box XP emulator--needed to
    ** limit the blocksize on fread() to < 2000000 (not sure of exact limit).     
    ** Happens during reading of eng.trained_data file.
    */
    {
    int blocksize,index,nleft;

    blocksize=65536;
    for (index=0,nleft=num_edges_;nleft > 0;)
        {
        int bsize;
        bsize = nleft > blocksize ? blocksize : nleft;
        fread(&edges_[index], sizeof(EDGE_RECORD), bsize, file);
        index += bsize;
        nleft -= bsize;
        }
    }
