...
static void pdf_load_system_font(fz_context *ctx, pdf_font_desc *fontdesc, char *fontname, char *collection, int has_encoding);
...
pdf_load_embedded_font(pdf_document *doc, pdf_font_desc *fontdesc, char *fontname, pdf_obj *stmref)
{
    ...
    buf=NULL; /* Avoid compiler warning--WILLUS */
	fz_try(ctx)
	{
...
pdf_free_font_imp(fz_context *ctx, fz_storable *fontdesc_)
{
	...
	/* pdf_ft_free_vsubst(fontdesc); */ /* WILLUS--disable this mod for now */
	if (fontdesc->font)
...
pdf_new_font_desc(fz_context *ctx)
{
    ...
    /* WILLUS:  Disable this mod for now */
    /*
